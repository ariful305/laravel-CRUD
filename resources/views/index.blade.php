    ...
    <table class="table table-striped table-bordered" style="width:100%;background-color:white;">
        ...
        <tbody> 
          ...
          @foreach ($Id as $key=>$iteam)
                ...
                <td>{{$iteam->phone}}</td>
                <td>{{$iteam->email}}</td>
                <td><img src="{{ asset($iteam->picture)}}" width="50px" height="50px" ></td>
                <td><a href="{{ route('show',$iteam->id)}}" class="btn btn-success" type="submit">view</a></td> 
                <td><a href="{{ route('edit',$iteam->id)}}" class="btn btn-primary" type="submit">Edit</a></td> 
                <td><a href="{{ route('delete',$iteam->id)}}" class="btn btn-primary" type="submit">delete</a></td> 
            </tr>
            @endforeach
